...
use crate::ax::{AXElement, AttributeValue, MENU_ITEM_ATTRS, attr_idx};
...
pub fn build_tree(pid: i32, max_depth: Option<usize>) -> Result<Vec<MenuNode>, MenuError> {
    ...
    std::thread::scope(|s| {
        let handles: Vec<_> = top_level
            ...
            .map(|(i, element)| {
                s.spawn(move || {
                    let node = walk_element(element, String::new(), 1, max_depth);
                    (i, node)
                })
            })
            .collect();
...
fn walk_element(
    ...
    let children = if max_depth.is_none_or(|max| depth < max) {
        collect_children(&element, &path, depth, max_depth)
    } else {
        Vec::new()
    };
...
fn collect_children(
    ...
    for child in child_elements {
        ...
        if role.as_deref() == Some("AXMenu") {
            ...
            let grandchildren = collect_children(&child, parent_path, parent_depth, max_depth);
            child_nodes.extend(grandchildren);
        } else if let Ok(node) =
            walk_element(child, parent_path.to_owned(), parent_depth + 1, max_depth)
        {
